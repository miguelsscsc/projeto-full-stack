...
export const createEnrollment = async ({
  ...
  const { rows } = await query(
    `INSERT INTO enrollments (student_id, subject_id, status, grade, attendance)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id`,
    [student_id, subject_id, status, grade, attendance]
  );
...
export const updateEnrollment = async (
  ...
  const { rows } = await query(
    `UPDATE enrollments
     SET student_id = $1, subject_id = $2, status = $3, grade = $4, attendance = $5
     WHERE id = $6
     RETURNING id`,
    [student_id, subject_id, status, grade, attendance, id]
  );
